...
#-----------------------
isovalue = 50
linear_interpolation = True # change to False to use midpoint method
...
# Linear interpolation to find the exact point on edge
def interpolate_point(p1, p2, v1, v2, isovalue):
    if abs(v2 - v1) > 0:
        t = (isovalue - v1) / (v2 - v1)
        return p1 + t * (p2 - p1)
    else:
...
def march_sq_lin_interp(data, i, j, isovalue):
    ...
    for line in lookup_table[index]:
        ...
        points = []
        if line[0] == 0.5:  # Horizontal edge
            if line[1] == 0:
                points.append(interpolate_point(p0, p3, data[i, j], data[i+1, j], isovalue))
            else:
                points.append(interpolate_point(p1, p2, data[i, j+1], data[i+1, j+1], isovalue))
        if line[2] == 0.5:  # Horizontal edge
            if line[3] == 0:
                points.append(interpolate_point(p0, p3, data[i, j], data[i+1, j], isovalue))
            else:
                points.append(interpolate_point(p1, p2, data[i, j+1], data[i+1, j+1], isovalue))
        if line[1] == 0.5:  # Vertical edge
            if line[0] == 0:
                points.append(interpolate_point(p0, p1, data[i, j], data[i, j+1], isovalue))
            else:
                points.append(interpolate_point(p3, p2, data[i+1, j], data[i+1, j+1], isovalue))
        if line[3] == 0.5:  # Vertical edge
            if line[2] == 0:
                points.append(interpolate_point(p0, p1, data[i, j], data[i, j+1], isovalue))
            else:
                points.append(interpolate_point(p3, p2, data[i+1, j], data[i+1, j+1], isovalue))

        # Draw line between interpolated points
        if len(points) == 2:
